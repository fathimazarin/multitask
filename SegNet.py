...
from evaluation_object import cal_loss, normal_loss, per_class_acc, get_hist, print_hist_summary, train_op, MAX_VOTE, var_calculate,segloss,multiloss
...
'''
            with ob[0].sess.as_default():
                with ob[1].sess.as_default():
                    with ob[2].sess.as_default():
                        
                    
                        with ob[3].sess.as_default():
                            ob[0].sess.run(tf.local_variables_initializer())
                            ob[0].sess.run(tf.global_variables_initializer()
                            ob[1].sess.run(tf.local_variables_initializer())
                            ob[1].sess.run(tf.global_variables_initializer())
                            ob[2].sess.run(tf.local_variables_initializer())
                            ob[2].sess.run(tf.global_variables_initializer())
                    	...
                    	with tf.Session() as sess:
                        	...
                        	for step in range(max_steps):
                            	...
                            	feed_dict = {ob[0].inputs_pl: image_batch,
                                 ...
                                 ob[0].with_dropout_pl: True,
                                 ob[0].batch_size_pl: batch_size,
                                 ob[1].inputs_pl: image_batch,
                                 ob[1].labels_pl: label_batch,
                                 ob[1].is_training_pl: True,
                                 ob[1].keep_prob_pl: 0.5,
                                 ob[1].with_dropout_pl: True,
                                 ob[1].batch_size_pl: batch_size,
                                 ob[2].inputs_pl: image_batch,
                                 ob[2].labels_pl: label_batch,
                                 ob[2].is_training_pl: True,
                                 ob[2].keep_prob_pl: 0.5,
                                 ob[2].with_dropout_pl: True,
                                 ob[2].batch_size_pl: batch_size,
                                 ob[3].inputs_pl: image_batch,
                                 ob[3].labels_pl: label_batch,
                                 ob[3].is_training_pl: True,
                                 ob[3].keep_prob_pl: 0.5,
...
'''
                    ...
                    if step % 1000 == 0:
                        ...
                        for test_step in range(int(20)):
                            fetches_valid = [loss, self.logits]
