...
def segloss(logits,labels):
    reconstruction_loss = tf.reduce_mean(tf.square(logits - labels)) # MSE 
    reg_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
    loss = tf.add_n([reconstruction_loss] + reg_losses)
    return loss

def multiloss(flayer,labels):
    reconstruction_loss = tf.reduce_mean(tf.square(flayer - labels)) # MSE 
    reg_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
    loss = tf.add_n([reconstruction_loss] + reg_losses)
    return loss
...
def train_op(seg1,seg2,seg3,seg4,lossf, opt):
    """
    Input:
    total_loss: The loss 
    ...
    with tf.control_dependencies(update_ops):
        ...
        grads1 = optimizer.compute_gradients(seg1, var_list=tf.trainable_variables())
        grads2 = optimizer.compute_gradients(seg2, var_list=tf.trainable_variables())
        grads3 = optimizer.compute_gradients(seg3, var_list=tf.trainable_variables())
        grads4 = optimizer.compute_gradients(seg4, var_list=tf.trainable_variables())
        gradsf = optimizer.compute_gradients(lossf, var_list=tf.trainable_variables())
        training_op1 = optimizer.apply_gradients(grads1, global_step=global_step)
        training_op2 = optimizer.apply_gradients(grads2, global_step=global_step)
        training_op3 = optimizer.apply_gradients(grads3, global_step=global_step)
        training_op4 = optimizer.apply_gradients(grads4, global_step=global_step)
        training_opf = optimizer.apply_gradients(gradsf, global_step=global_step)

    return training_op1,training_op2,training_op3,training_op4,training_opf, global_step
